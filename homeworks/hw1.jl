...
# ╔═╡ 89467880-0fe0-11eb-3c86-9fb19d14ae8b
...
# ╔═╡ f0a530e0-007a-11eb-33cb-45e23b1dc162
...
# ╔═╡ 4a92f110-011f-11eb-27fc-4964837daa81
typeof(vecvec_to_matrix([[1,2], [3,4]]))

# ╔═╡ 56974420-011f-11eb-0be9-1b6c94f59e4c
...
function matrix_to_vecvec(matrix)
	...
	shape = size(matrix)
	return vcat( [matrix[i,:] for i in 1:shape[1]] )
end
...
# ╔═╡ 6b07a420-0126-11eb-2239-efb28b18d589
...
begin
	...
	function quantize(color::AbstractRGB)
		...
		new_color = RGB(quantize(color.r),
			quantize(color.g),
			quantize(color.b))
...
begin
	function noisify(x::Number, s)
		...
		# Tranform [0,1]  to [-s, s]
		random_value = 2*s* rand(Float64,(1,1))[1] - s
		
		new_x = x + random_value
	...
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		r,g,b = noisify(color.r,s),noisify(color.g,s),noisify(color.b,s)
		new_rgb = RGB(r,g,b)
		return new_rgb
	end
...
# ╔═╡ 6a64ff90-02a7-11eb-1729-ebef2dbe6749
rand(Float64, (1,1))


# ╔═╡ 85ef5260-02a7-11eb-311b-ad5dbe7fabd5
rand(Float64, (1,1))[1]
...
# ╔═╡ ac370aae-0989-11eb-2543-a90589850127
typeof(v)
...
function blur_1D(v, l)
	n = size(v)[1]
	new_v = Array{Float64,1}(undef, n )
	if l== 0
		return v
	end
	for i = 1:n
		new_v[i] = (1/l)*sum([extend(v,k) for k =i-l:i+l ])
...
# ╔═╡ a6635ae0-098d-11eb-2bab-ddc94a63318a
@bind l_box  Slider(0:1:100, show_value=true)


# ╔═╡ 186b8e00-098e-11eb-346b-e55a8bc3f0d0
 colored_line(v)
...
function convolve_vector(v, k)
	...
	#if kernel with even lenght
	if n%2 == 0
		ArgumentError("Length of kernel must be odd. Get:" * string(n))
	end
	
	
	for i=1:n
		new_v[i] = sum([extend(v,i+j)*k[j+(l+1)] for j=-l:l ])
	end
...
# ╔═╡ ba4480f0-0999-11eb-3cbf-4ba31d1bc026
string(2) * " Aha"
...
# ╔═╡ c15f8100-0999-11eb-27f3-5b0810a2e103
...
# ╔═╡ cd8bce70-0999-11eb-05f4-33117d9f3c6e
test_convolution3 = let
	v = [1, 10, 100, 1000, 10000]
	k = [0.9, 0, 0.1]
	convolve_vector(v, k)
end
...
# ╔═╡ df18f360-099a-11eb-04f3-8b895bc340a0
function gaussian_value(x)
...
function gaussian_kernel(n)
	...
	#construct one side Gaussian array
	g_array = [gaussian_value(i) for i=1:l]
	...
	# Assing left values on reverse order
	vec[l:-1:1] = g_array
...
# ╔═╡ de368e90-09da-11eb-3bd3-ef91f52adb93
begin 
...
# ╔═╡ 074b4bf0-099e-11eb-1a48-731daac9a33c
gaussian_kernel(3)
...
@bind gaussian_kernel_size_1D  Slider(1:2:11, show_value=true) # change this value, or turn me into a slider!
...
# ╔═╡ 553ee180-0a5e-11eb-35ae-0d3e2c6ec259
K = [1 2 3 ;
	 4 5 6 ;
	 7 8 9]

# ╔═╡ 2570b8c0-0a5e-11eb-2d1e-fd1c4f1f6239
begin 
	extend_mat(K, 2,5)
end
...
# ╔═╡ baa51e92-0fdb-11eb-2089-ab1bf2890e21
function get_2d_gaussian(n)
	
	
	y = Array{Float64, 2}(undef,n,n)
	y[:,1:n] .=LinRange(-(n - 1)/2, (n - 1)/2, n)
	
	x = Array{Float64, 2}(undef,n,n)
	x[1:n,:] .=LinRange(-(n - 1)/2, (n - 1)/2, n)'
...
# ╔═╡ f21db620-0fdb-11eb-2c45-4143aff52ac7
gk = get_2d_gaussian(3)

# ╔═╡ f2db9c60-1032-11eb-1187-f12c63da630c
begin 
	r,c = size(gk)
	r,c
...
# ╔═╡ 9042f410-0fe0-11eb-37da-951cb1ffc4ef

begin 
	ker = ImageFiltering.Kernel.gaussian((1,1))
	size(ker)
	ker
end

# ╔═╡ 5158bbc0-0a60-11eb-31a4-a395db1441fe
function convolve_pixel(image,x,y, kernel)
	
	rows,columns = size(kernel)
	
	r,g,b= 0.0,0.0,0.0
	x_dis = Integer(ceil(rows/2))
	y_dis = Integer(ceil(columns/2))
	
	for i= 1:rows
		for j= 1:columns
			pixel = extend_mat(image,
				x-x_dis + i,
				y-y_dis + j)
...
# ╔═╡ 38e201f0-103c-11eb-00eb-092deef8a29f
15%10

# ╔═╡ 8d874e7e-1038-11eb-0ce8-9186fa9930b5
gk

# ╔═╡ cc240dde-1038-11eb-085d-bfd959ecff2c
begin 
	a= 0
	a+= 0.0001
	a+= 0.00001
...
# ╔═╡ 727829b2-1039-11eb-3fc7-ef2610c19a50
RGB(1.0,1.0,2.0)
...
function with_gaussian_blur(image)
	...
	new_image = Array{RGB{Normed{UInt8,8}},2}(undef, r,c)
	kernel = get_2d_gaussian(3)
	
	for i=1:r
		for j= 1:c
			new_image[i,j] = convolve_pixel(image,i,j,kernel)
		end
	end
...
\otimes
```
is the Outer product of vectors
"""

# ╔═╡ 7ff29530-10ed-11eb-107d-9bcb9a2642ab
begin 
	size([1; 2; 2])
	size([1 2 3 ])
end 

# ╔═╡ 753a4a20-10ed-11eb-1b25-37a6305dc690
function get_gx()
	return [1;2;1]*[1 0 -1]
end
	

# ╔═╡ 74ac13e0-10ed-11eb-3fa0-075333be8606
get_gx()

# ╔═╡ 43437f90-10ee-11eb-3909-3369c05767cd
function get_gy()
	return [1; 0; -1] * [1 2 1]
end

# ╔═╡ 665528d2-10ee-11eb-17aa-47f1f74ee1a5
get_gy()

# ╔═╡ 8f47fd20-10ef-11eb-100d-3ba609626394
function  apply_convolution(img, kernel)
	rows, cols = size(img)
	new_image = Array{RGB{Normed{UInt8,8}},2}(undef, rows, cols )
	
	for i= 1:rows
		for j=1:cols
			new_image[i,j] = convolve_pixel(img,i,j,kernel)
		end
	end
...
function with_sobel_edge_detect(image)
	...
	gradient_x = apply_convolution(image,gx)
	gradient_y = apply_convolution(image,gy)
	
	edges = Array{Float64,2}(undef, rows, cols)
	
	for i= 1:rows
		for j=1:cols
			edges[i,j]= sqrt(gradient_x[i,j].r^2 + gradient_y[i,j].r^2)
		end
	end
...
# ╔═╡ ce5cecae-012b-11eb-3c8e-4d4bdac63177
typeof(philip)

# ╔═╡ 7946bd40-012c-11eb-03e8-17d165e57fa6
...
# ╔═╡ f4b9d60e-0fe1-11eb-1a20-ab98bcc9d59f
typeof(extend_mat(philip,1,1))

# ╔═╡ 734f1e30-1038-11eb-367b-5bc2287f1389
philip[1,1].r,philip[1,1].g,philip[1,1].b

# ╔═╡ afb0a580-1036-11eb-3254-25ab8f63da14
pixel_ = convolve_pixel(philip,1,1,gk)

# ╔═╡ d050b27e-1036-11eb-04f2-7d6980f5b894
pixel_.r,pixel_.g,pixel_.b

# ╔═╡ fe1ebf20-102e-11eb-38de-ffd9ba75f56d
size(philip)

# ╔═╡ 36e5b000-102c-11eb-1a40-c39f0787cc62
gaussian_philip = with_gaussian_blur(philip)
...
# ╔═╡ 5ce15bf0-10ff-11eb-0a8c-8dcf21c0f948
colored_line(edges)

# ╔═╡ 8d2a6bf2-10fd-11eb-104a-c1fc712af345
view(convert(Image, map(ScaleAutoMinMax(U8),edges)))

# ╔═╡ c35ca0d0-10fd-11eb-3e51-55341d57f9f2
img=convert(Image,edges)

# ╔═╡ 72a02400-10fd-11eb-39cd-2b30bc7d0cba
view(convert(Image, map(ScaleAutoMinMax(U8),data)))
...
# ╔═╡ b6ce9740-10f0-11eb-174b-95453c5d1ef5
edges = with_sobel_edge_detect(philip)

# ╔═╡ d1e45880-10ff-11eb-0a46-99c1c830dbac
begin
	import Pkg 
	Pkg.add("PGFPlots")
	using Plots; pgfplots()
	heatmap(edges, c = :greys)
end
